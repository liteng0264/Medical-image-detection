...
print(test_db)
#网络
conv_layers = [layers.Conv2D(64,kernel_size=[3,3],padding="valid",activation=tf.nn.relu),
...
for x,y in test_db:
    ...
    #pred = tf.cast(pred,dtype=tf.int32)  #张量数据类型转换
    
    pred = tf.where(logits>0.5,1,0)
    pred = tf.cast(pred,dtype=tf.int32)  #张量数据类型转换
    y = tf.cast(y,dtype=tf.int32)  #张量数据类型转换
