...
TrainX_rgb = np.zeros([totalfiles ,224,224]) #图片
...
for filename in filenames:
    ...
    if basefilename[0:1] == 'B':
        TrainY_rgb[fileindex] = 1 # CD
    else: 
        if basefilename[0:1] == 'A':
...
'''
network = tf.keras.Sequential([tf.keras.layers.Conv2D(32, (3,3), padding='same', activation=tf.nn.relu,
